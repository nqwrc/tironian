...
- Code quality stays red on failures inherited from the fork (18 on `feature/whispering-snippets`, run 34218177933) plus a local-books test timeout. Every failure the rename caused is fixed.

## next
- Nicola's decisions: sign-in and sync go to api.epicenter.so; Apple Developer ID and Windows signing purchases; accent `#D97757`; Honeycrisp still listed in Home; KPIs and M0 evidence for this repo.
