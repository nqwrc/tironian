...
- Vivavoce design, deliverable 4 (project "App redesign directions" on Claude Design, 10 screens): deliverable 2 tokens already ship in `brand.css`. Done on `feature/vivavoce-shell`: the labeled sidebar (4b shell), open by default at 13rem, Settings pinned at the bottom with the theme toggle, and a stats footer (wpm dictated and median wait from the newest 50 completed recordings, plus the local or cloud route; the design's "% local" is not computable because rows do not record the service), seen in Tironian Dev over CDP. Not implemented yet: Home 4b, Registrazioni rows 4c, six-tab Settings 4d/4j, Dettatura 4f (Polish, commands, dictionary), Ricette 4g, Snippet 4h, detail modal 4i, light onboarding 4e, pill states 4a. Not carried over, per docs/brand/tironian.md: the vivavoce wordmark, the soundwave mark, Italian as source language.
