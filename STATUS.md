...
- `bugfix/staging-sweep-liveness` (cut from `feature/single-window`): the recorder's startup sweep deletes a staged capture only when the pid in its name has exited, so Tironian Dev no longer deletes production's in-flight audio from the shared data root. The root stays shared by design (dev and prod both resolve `app.tironian`), so there is no ADR. `cargo test` 120/0 (was 116, plus 4 sweep and probe tests). Built with `CARGO_TARGET_DIR=C:\ct` because D: had 0.59 GB free (`D:\ct` holds 17.8 GB). Not yet verified: a live dev launch while prod is recording.
