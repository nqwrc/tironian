...
- `feature/ctrl-win-push-to-talk` (ADR-0246, stacked on #3): on Windows, push-to-talk defaults to a Ctrl+Win hold detected by a low-level keyboard hook that never swallows keys, masks Start with `0xE8`, and releases on any third key; macOS and Linux keep their chords, and a stored chord outranks the new default. Dictation suite 326/0, typecheck 0 errors. Owed: a real Ctrl+Win hold in the installed app, which needs physical keys.
