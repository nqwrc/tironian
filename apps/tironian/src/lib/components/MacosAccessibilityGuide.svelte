<script lang="ts">
	...
	//   - `first-grant` (never granted): `openSystemSettings` already adds Tironian
	//     to the list toggled off, so the whole job is flipping its switch. Telling a
	//     brand-new user to "remove Tironian" is impossible: it isn't there yet.
...
<ol class="flex flex-col gap-3">
	...
	{#if variant === 're-add'}
		{@render step(1, removeTironian)}
		{@render step(2, readdTironian)}
	{:else}
		{@render step(1, findTironian)}
...
{#snippet findTironian()}
	Find {@render term('Tironian')} in the list (we added it for you).
{/snippet}
...
{#snippet removeTironian()}
	Click {@render term('Tironian')}, then remove it with the {@render control(
		'−',
	)} button.
{/snippet}

{#snippet readdTironian()}
