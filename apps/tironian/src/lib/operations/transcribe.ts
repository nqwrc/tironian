...
const log = createLogger('tironian/transcribe');
...
const TranscriptionOperationError = defineErrors({
	...
	/** Tironian already knows this cannot succeed: nothing is selected, or the
 ...
 * ADR-0060 blesses it.
 */
const uploadDispatch = (app: TironianApp) =>
	({
		OpenAI: {
			kind: 'wire',
			resolve: () =>
...
async function isSilent(
	...
	const speech = await containsSpeech({
		audio,
		assetBaseUrl: `${DICTATION_BASE_PATHNAME}/vad/`,
	});
	if (!speech) {
 ...
 */
export function prewarmOnDeviceModel(app: TironianApp): void {
	if (!tauri) return;
 ...
 *
 * Tironian names audio and advisory hints; it does not name a model
