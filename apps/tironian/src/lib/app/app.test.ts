/**
 * Tironian app acquisition tests.
 *
...
import { openTironianApp, type TironianAppDependencies } from './app';
...
const dependencies: TironianAppDependencies = {
	blobs: { local },
};
...
test('settings recover application defaults and survive a restart', async () => {
	...
	await using reopened = await openTironianApp(dependencies);
...
test('a stored tironian transcriptionService reads back as local', async () => {
	await resetStorage();
	await using app = await openTironianApp(dependencies);
	// A row written before hosted transcription was removed: the closed
	// `field.select` no longer admits 'tironian', so conformance drops just
	// this one key and the application default takes its place.
	app.settings.set('transcriptionService', 'tironian' as never);
	await Bun.sleep(10);
	expect(app.settings.get('transcriptionService')).toBe('local');
});
...
test('an aborted boot rejects with the abort', async () => {
	...
	expect(
		openTironianApp(dependencies, { signal: controller.signal }),
