...
import { type RecordingId, tironianDefinition } from '../workspace';
import { asStoredBlobId, type NewRecording } from './recording';
import { createTironianRecordings } from './recordings';
...
async function setup({
	...
	const root = mkdtempSync(join(tmpdir(), 'tironian-recordings-'));
	const opened = await open(tironianDefinition, { root });
	if (opened.error !== null) throw opened.error;
	const data = opened.data;
	const table = data.tables.recordings;
	for (const row of seed) expectOk(table.create(storedRow(row)));
	const domain = createTironianRecordings({
		table,
		blobs: { local },
	});
	return {
