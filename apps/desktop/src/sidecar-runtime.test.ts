...
describe('runtime mode', () => {
	test('source and compiled argv shapes select the Rust-supplied mode', () => {
		...
		expect(
			parseRuntimeMode(['/app/tironian-bun', '--runtime-mode=development']),
		).toBe('development');
	});

	test('missing, unknown, and additional arguments are rejected', () => {
		expect(() => parseRuntimeMode(['/app/tironian-bun'])).toThrow(
			'exactly one',
		);
		expect(() =>
			parseRuntimeMode(['/app/tironian-bun', '--runtime-mode=test']),
