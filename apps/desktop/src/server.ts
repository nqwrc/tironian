/**
 * The Bun-owned Tironian origin: trusted SPA documents and local blob
...
function validateOrigin(origin: string): URL {
	...
	try {
		...
		throw new Error(`Invalid Tironian origin: ${origin}`);
	}
	if (
		url.origin !== origin ||
		url.protocol !== 'http:' ||
		...
		throw new Error(
			'Tironian origin must be exact http://127.0.0.1:<port> without credentials or a path.',
		);
	}
	return url;
}
...
function contentSecurityPolicy(page: string): string {
	...
	return [
		...
		// onnxruntime in this WebView over assets Tironian itself ships, so
