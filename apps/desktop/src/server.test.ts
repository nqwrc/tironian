...
function applicationAssets(assets: TironianStaticAssets, id: string) {
...
function writeAppsDistFixture(homePage: string = PAGE): string {
	...
	mkdirSync(join(root, 'dictation', 'vad'), { recursive: true });
	writeFileSync(
		join(root, 'dictation', '_app', 'immutable', 'entry.js'),
		'window.dictationLoaded = true;',
	);
	writeFileSync(
		join(root, 'dictation', 'vad', 'silero_vad_v5.onnx'),
		'vad-model',
	);
	...
	writeFileSync(
		join(root, 'dictation', 'vad', 'ort-wasm-simd-threaded.wasm'),
		'\0asm\x01\0\0\0',
	);
	return root;
}
...
describe('loadStaticAssets', () => {
	...
	test('resolves nested generated assets and extensionless SPA routes', async () => {
		...
		const dictation = applicationAssets(assets, 'dictation');
		const nested = await dictation.resolve(
			'/apps/dictation/_app/immutable/entry.js',
		);
		expect(nested?.contentType).toContain('text/javascript');
		expect(await nested?.file.text()).toContain('dictationLoaded');

		const vad = await dictation.resolve(
			'/apps/dictation/vad/silero_vad_v5.onnx',
		);
		expect(await vad?.file.text()).toBe('vad-model');

		const fallback = await dictation.resolve(
			'/apps/dictation/settings/transcription',
		);
		expect(await fallback?.file.text()).toBe(DICTATION_PAGE);
		expect(
			await dictation.resolve('/apps/dictation/_app/missing.js'),
		).toBeUndefined();
	});

	test('rejects raw, encoded, double-encoded, and symlink traversal', async () => {
		const root = writeAppsDistFixture();
		const outside = mkdtempSync(join(tmpdir(), 'tironian-outside-assets-'));
		writeFileSync(join(outside, 'secret.txt'), 'outside secret');
		symlinkSync(
			join(outside, 'secret.txt'),
			join(root, 'dictation', 'linked-secret.txt'),
		);
		symlinkSync(
			join(outside, 'secret.txt'),
			join(root, 'dictation', 'linked-secret'),
		);
		const assets = await loadStaticAssets(root, COMPILED_APPLICATIONS);
		const dictation = applicationAssets(assets, 'dictation');

		for (const pathname of [
			'/apps/dictation/../home/index.html',
			'/apps/dictation/%2e%2e/home/index.html',
			'/apps/dictation/%252e%252e/home/index.html',
			'/apps/dictation/%2fetc/passwd',
			'/apps/dictation/%252fetc/passwd',
			'/apps/dictation//etc/passwd',
			'/apps/dictation/..\\query\\index.html',
			'/apps/dictation/%00index.html',
			'/apps/dictation/linked-secret.txt',
			'/apps/dictation/linked-secret',
		]) {
			expect(await dictation.resolve(pathname)).toBeUndefined();
		}
	});
});
...
describe('createHomeServer', () => {
	...
	test('serves Home and every compiled application', async () => {
		...
		try {
			...
			expect(dictationPage).toBe(DICTATION_PAGE);
			const dictationAsset = await fetch(
				`${server.url.origin}/apps/dictation/_app/immutable/entry.js?v=1`,
				{ headers: authenticatedHeaders(server) },
			);
			expect(await dictationAsset.text()).toContain('dictationLoaded');
			expect(dictationAsset.headers.get('content-type')).toContain(
				'text/javascript',
			);
			const vadAsset = await fetch(
				`${server.url.origin}/apps/dictation/vad/silero_vad_v5.onnx`,
				{ headers: authenticatedHeaders(server) },
			);
			expect(await vadAsset.text()).toBe('vad-model');
			const clientRoute = await fetch(
				`${server.url.origin}/apps/dictation/settings/transcription?tab=models`,
				{ headers: authenticatedHeaders(server) },
			);
	...
	test('admits first-party WebAssembly without restoring eval', async () => {
		...
		try {
			...
			const wasm = await fetch(
				`${server.url.origin}/apps/dictation/vad/ort-wasm-simd-threaded.wasm`,
				{ headers: authenticatedHeaders(server) },
			);
	...
	test('the account broker is gone, not just gated', async () => {
		const server = await serveHost();
		const { cookie, origin } = authenticationFor(server);
		try {
			// The account broker was removed along with sign-in and sync. Probe
			// the host infrastructure prefix, derived from BOOTSTRAP_ROUTE so a
			// future rename of that prefix cannot leave this test probing a
			// path the server no longer serves anything under.
			...
			for (const path of ['sign-in', 'sign-out', 'instance', 'profile']) {
				const response = await fetch(`${origin}${livePrefix}/${path}`, {
					method: 'POST',
...
describe('local blob routes', () => {
	...
	test('remote copy routes are gone, not just gated', async () => {
		const server = await serveHost();
		const id = generateBlobId();
		const { cookie, origin } = authenticationFor(server);
		try {
...
describe('sidecar end-to-end smoke', () => {
	test('the spawned entrypoint announces readiness and serves the built SPA', async () => {
		...
		try {
			// The Rust-resolved port travels in the boot frame.
			sidecar.stdin.write(
				`${JSON.stringify({ type: 'boot', protocolVersion: SIDECAR_PROTOCOL_VERSION, token: TOKEN, port })}\n`,
			);
	...
	test('a port collision exits without announcing readiness or falling back', async () => {
		...
		try {
			sidecar.stdin.write(
				`${JSON.stringify({ type: 'boot', protocolVersion: SIDECAR_PROTOCOL_VERSION, token: TOKEN, port: occupiedPort })}\n`,
			);
			await sidecar.stdin.flush();
