...
async function main(): Promise<void> {
	...
	try {
		...
		// The one Tironian root, resolved here rather than received. A desktop
		...
		// (ADR-0227).
		const dataRoot = tironianDataRoot();
		...
		if (!appsDist) {
			throw new Error(
				'TIRONIAN_APPS_DIST must name the release-built Tironian applications directory.',
			);
		}
