...
//! `tironian_lib::timing` target when `TIRONIAN_TIMING` is set in the
...
//!
//! ```sh
//! TIRONIAN_TIMING=1 bun run --cwd apps/desktop tauri dev 2>&1 | grep '\[timing\]'
//! ```
//!
...
/// Whether timing logs are enabled. Read once from `TIRONIAN_TIMING`; any
/// value (even empty) enables it. Cached so the hot path is a single load.
pub fn enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| std::env::var_os("TIRONIAN_TIMING").is_some())
}
...
pub fn note(message: std::fmt::Arguments<'_>) {
    if enabled() {
        log::info!(target: "tironian_lib::timing", "[timing] {message}");
    }
}

/// `log::info!`-style note on the `tironian_lib::timing` target, gated by
/// `TIRONIAN_TIMING`. The format arguments are evaluated only when timing
/// is enabled.
#[macro_export]
