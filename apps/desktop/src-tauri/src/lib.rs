...
pub fn run() {
    ...
    builder
        ...
        .expect("failed to build Tironian")
        .run(|app, event| match event {
            ...
            #[cfg(target_os = "macos")]
            RunEvent::Reopen { .. } => request_window(app, BuiltInApp::Dictation),
...
fn open_forwarded_deep_links(app: &DesktopAppHandle, arguments: &[String]) {
    ...
    if built_ins.is_empty() {
        if !launched_from_autostart(arguments) {
            request_window(app, BuiltInApp::Dictation);
        }
    } else {
...
fn start_once(app: &DesktopAppHandle) -> Result<()> {
...

    {
        ...
        if process.is_some() {
            ...
            bail!("a Bun host is already managed by Tironian");
        }
...
fn launch_host(app: &DesktopAppHandle, port: u16) -> Result<LaunchedHost> {
    ...
    // The Bun host resolves the Tironian data root itself, from the one
...
fn apps_dist(_app: &DesktopAppHandle) -> Result<PathBuf> {
    Ok(std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .context("Tironian src-tauri directory has no app parent")?
        .join("dist"))
}
...
#[cfg(not(debug_assertions))]
fn host_command(_app: &DesktopAppHandle) -> Result<Command> {
    let executable = std::env::current_exe().context("resolve the Tironian executable")?;
    let directory = executable
        .parent()
        .context("the Tironian executable has no parent directory")?;
...
fn create_windows_on_main_thread(
    ...
    app.clone().run_on_main_thread(move || {
        let result = (|| {
            ...
            ensure_window(&app, BuiltInApp::Dictation, port, &token, false)?;
    ...
    receiver
        .recv()
        .context("the main thread stopped before creating Tironian windows")?
}

#[cfg(any(target_os = "macos", target_os = "windows"))]
fn create_recording_overlay(app: &DesktopAppHandle, port: u16, token: &str) -> Result<()> {
    let origin = origin(port);
    let url: tauri::Url = format!("{origin}/apps/dictation/recording-overlay/").parse()?;
    ...
    overlay::create_recording_overlay(app, url, initialization_script, port)
        .context("create the Dictation recording overlay")
}
...
/// an app owning a second window (Dictation's recording overlay) creates it
...
fn ensure_log_file(app: &DesktopAppHandle) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("create Tironian log directory at {}", parent.display()))?;
    }
    OpenOptions::new()
        .create(true)
        .append(true)
        .open(&path)
        .with_context(|| format!("open Tironian host log at {}", path.display()))?;
    Ok(())
}
...
fn open_log_file(app: &DesktopAppHandle) -> Result<File> {
    ...
    OpenOptions::new()
        ...
        .append(true)
        .open(log_path(app)?)
        .context("open the stable Tironian host log")
}
...
fn initialization_script(origin: &str, token: &str) -> Result<String> {
  ...
  const sessionReady = fetch('/_tironian/bootstrap', {{
    ...
    if (!response.ok) throw new Error(`Tironian session bootstrap failed (${{response.status}}).`);
  }});
...
mod tests {
    ...
    fn trusted_app_capabilities_grant_the_shared_http_slice() {
        for encoded in [
            ...
            assert_eq!(
                ...
                "the trusted-app HTTP slice belongs to Dictation alone now that no catalog admits other apps"
            );
    ...
    #[test]
    fn dictation_native_capabilities_do_not_duplicate_trusted_app_http() {
        for encoded in [
            ...
            assert!(
                ...
                "trusted-app HTTP belongs to the shared app capability, not Dictation native"
            );
        }
    }

    /// The `tauri-plugin-macos-permissions` plugin is gone from this build:
    /// `command.rs` owns the two OS permission capabilities Tironian exposes,
    /// through AVFoundation and the Accessibility API directly.
    ///
    /// Its `macos-permissions:default` grant handed Dictation twelve unrelated
    ...
    /// Dictation windows are checked, not just the one that had it.
    #[test]
    fn no_dictation_capability_grants_the_deleted_permissions_plugin() {
        for encoded in [
            include_str!("../capabilities/trusted-dictation-native-development.json"),
            include_str!("../capabilities/trusted-dictation-native-production.json"),
            include_str!("../capabilities/trusted-dictation-overlay-development.json"),
            include_str!("../capabilities/trusted-dictation-overlay-production.json"),
        ] {
            let capability: serde_json::Value = serde_json::from_str(encoded).unwrap();
