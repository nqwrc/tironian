...
Tironian uses two active license tiers, split by how you use the code rather than by package type: code you *run* (both apps) is AGPL-3.0, and code you *build with* (the embeddable toolkit: `data`, `ui`, `sqlite`, and their internal utilities) is MIT to maximize developer adoption. A third proprietary tier is documented as an escape hatch but deferred indefinitely; revenue comes from hosting, not licensing. There is no Contributor License Agreement; we do not dual-license.
